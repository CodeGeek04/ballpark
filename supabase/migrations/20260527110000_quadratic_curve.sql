-- Switch scoring curve from linear to quadratic decay. The linear curve
-- penalized "kinda close" guesses too harshly (10x off → 333 points). The
-- quadratic curve rewards being in the ballpark much more heavily.
--
--   score = max(0, round(1000 * (1 - (min(1, |log10(guess/answer)| / k))^2)))
--
-- With k=2: exact=1000, 2x off=977, 5x off=878, 10x off=750, 100x off=0.

update public.questions set k = 2 where k = 1.5;
alter table public.questions alter column k set default 2;
...
begin
  ...
  if v_round.revealed_at is null then
    ...
    set score = greatest(
      0,
      round(1000 * (1 - power(least(1, abs(log(s.guess / v_question.answer)) / v_question.k), 2)))::int
    )
...
begin
  ...
  if v_revealed_at is not null and new.score is null then
    if new.guess > 0 and v_answer > 0 then
      new.score := greatest(
        0,
        round(1000 * (1 - power(least(1, abs(log(new.guess / v_answer)) / v_k), 2)))::int
      );
    else
      new.score := 0;
    end if;
...
-- Retroactively re-score every existing submission with the new curve so
-- past scores match what the live game will produce from now on.
update public.submissions s
set score = greatest(
  0,
  case
    when s.guess > 0 and q.answer > 0
    then round(1000 * (1 - power(least(1, abs(log(s.guess / q.answer)) / q.k), 2)))::int
    else 0
  end
)
from public.rounds r
